...
try {
    ...
    if (!isset($_POST['id']) || empty($_POST['id'])) {
        throw new Exception("ID de peritaje no especificado");
    }
    ...
    for ($i = 1; $i <= 6; $i++) {
        ...
        if (isset($_FILES[$file_key]) && $_FILES[$file_key]['error'] === UPLOAD_ERR_OK && $_FILES[$file_key]['size'] > 0) {
            ...
            if (isset($_POST[$current_key]) && !empty($_POST[$current_key])) {
                $old_file = $uploadDir . $_POST[$current_key];
        ...
        else if (isset($_POST[$current_key]) && !empty($_POST[$current_key])) {
            $uploaded_files[$file_key] = $_POST[$current_key];
        } 
        // No hay imagen
        else {
    ...
    // Construir los campos y valores para el UPDATE
    $columnas = [
    ...
    foreach ($columnas as $columna) {
        ...
        if (strpos($columna, 'fijacion_fotografica_') === 0) {
            $valores[] = $uploaded_files[$columna];
        } else {
            $valores[] = $_POST[$columna] ?? null;
        }
    }
    ...
    // Debug para verificar el SQL y la cantidad de valores
    $query = "UPDATE peritaje_completo SET " . implode(', ', $set_parts) . " WHERE id = ?";
    error_log("SQL UPDATE: " . $query);
    error_log("Total columnas: " . count($columnas) . ", Total valores: " . count($valores));
    ...
    // Crear string de tipos de parámetros - todos como string para simplificar
    $types = str_repeat('s', count($valores));
    
    // Bind parámetros y ejecutar
    ...
    // Actualizar tablas relacionadas - primero eliminar registros existentes
    // 1. Eliminar y recrear inspección visual carrocería
    $stmt = $conn->prepare("DELETE FROM inspeccion_visual_carroceria WHERE peritaje_id = ?");
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $stmt->close();

    // Insertar nuevos registros de inspección visual carrocería
    if (
        ...
        for ($i = 0; $i < count($_POST['descripcion_pieza']); $i++) {
            ...
            if (!empty($descripcion) || !empty($concepto)) {
                $stmtInspeccion->bind_param('iss', $id, $descripcion, $concepto);
    ...
    // 2. Eliminar y recrear inspección visual estructura
    $stmt = $conn->prepare("DELETE FROM inspeccion_visual_estructura WHERE peritaje_id = ?");
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $stmt->close();

    // Insertar nuevos registros de inspección visual estructura
    if (
        ...
        for ($i = 0; $i < count($_POST['descripcion_pieza_estructura']); $i++) {
            ...
            if (!empty($descripcion) || !empty($concepto)) {
                $stmtEstructura->bind_param('iss', $id, $descripcion, $concepto);
    ...
    // 3. Eliminar y recrear inspección visual chasis
    $stmt = $conn->prepare("DELETE FROM inspeccion_visual_chasis WHERE peritaje_id = ?");
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $stmt->close();

    // Insertar nuevos registros de inspección visual chasis
    if (
        ...
        for ($i = 0; $i < count($_POST['descripcion_pieza_chasis']); $i++) {
            ...
            if (!empty($descripcion) || !empty($concepto)) {
                $stmtChasis->bind_param('iss', $id, $descripcion, $concepto);
    ...
    $_SESSION['success'] = "Peritaje actualizado correctamente";
    ...
    error_log("Error en actualización de peritaje completo: " . $e->getMessage());
    ...
    // También establecer en la sesión para ser mostrado después del redireccionamiento
    $_SESSION['error'] = "Error: " . $e->getMessage();
    
    // Comentado para mostrar errores en pantalla, descomentar para producción:
    // header('Location: ../E_peritajeC.php?id=' . $id);
    exit;

} finally {
    // Cerrar todas las conexiones
    if (isset($stmtInspeccion)) $stmtInspeccion->close();
    if (isset($stmtEstructura)) $stmtEstructura->close();
    if (isset($stmtChasis)) $stmtChasis->close();
